...
- `nova add [type] [name]` — scaffolds a single component (`entity`, `usecase`, `handler`, `repository`, `worker`, or `all`) into an existing project; interactive when run with no args (aliases: `generate`, `gen`)

Module is `github.com/quyennguyenvu/nova`, binary is `bin/nova`. Requires Go 1.25.1+.
...
- [docs/](docs/README.md) is the design reference for the generated project — consult it when a template's intent is unclear. Where a doc and a template disagree, the template wins and the doc is the bug; fix it in the same change.
